...
class BiLSTM_CRF(nn.Module):
    ...
    def _score_sentence(self, feats, tags):
        ...
        tags = torch.cat([torch.tensor([self.tag_to_ix[config.START_TAG]], dtype=torch.long).cuda(), tags])
    ...
    def accuracy_fn(self,pred_tag, target_tag):
        score = 0.0
        total = 0
        for i in range(len(target_tag)):
            if target_tag[i] !=0:
                total += 1
                if pred_tag[i] == target_tag[i]:
                    score +=1
        return score/total
    ...
    def neg_log_likelihood(self, sentence, tags):
        ...
        accuracy_score = self.accuracy_fn(tag_seq,tags)
